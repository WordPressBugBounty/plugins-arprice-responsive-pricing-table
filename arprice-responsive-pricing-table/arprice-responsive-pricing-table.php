...
class ARPlite_PricingTable {
	...
	function __construct() {
		...
		add_action( 'login_enqueue_scripts', array( $this, 'arplite_login_footer' ), 99 );
	...
	function arplite_login_footer() {
		...
		global $arpricelite_version;
		wp_enqueue_script( 'arplite-admin-notice-script', ARPLITE_PRICINGTABLE_URL . '/js/arplite-admin-notice.js', array(), $arpricelite_version, true );

		wp_add_inline_script( 'arplite-admin-notice-script', $arplite_script );
		...
		if ( is_admin() ) {
			...
			if ( isset( $_GET['page'] ) && 'arprice' == $_GET['page'] ) {
				...
				if ( ! isset( $_GET['arplite_page_nonce'] ) ) {
					...
					wp_redirect( $url );
					die;
				} elseif ( isset( $_GET['arplite_page_nonce'] ) && ! wp_verify_nonce( sanitize_text_field( $_GET['arplite_page_nonce'] ), 'arplite_page_nonce' ) ) {
				...
				if ( ! isset( $_GET['arplite_import_export_nonce'] ) ) {
					...
					wp_redirect( $url );
					die;
				} elseif ( isset( $_GET['arplite_import_export_nonce'] ) && ! wp_verify_nonce( sanitize_text_field( $_GET['arplite_import_export_nonce'] ), 'arplite_import_export_nonce' ) ) {
					wp_die( 'Sorry, the page you are trying to access is not accessible due to security reason.' );
				}
				...
				if ( ! isset( $_GET['arplite_global_settings_nonce'] ) ) {
					...
					wp_redirect( $url );
					die;
				} elseif ( isset( $_GET['arplite_global_settings_nonce'] ) && ! wp_verify_nonce( sanitize_text_field( $_GET['arplite_global_settings_nonce'] ), 'arplite_global_settings_nonce' ) ) {
					wp_die( 'Sorry, the page you are trying to access is not accessible due to security reason.' );
				}
	...
	function arplite_gutenberg_capability() {
		global $wpdb, $arpricelite_assset_version;

		wp_register_script( 'arprice_lite_script_for_gutenberg', ARPLITEURL . '/js/arprice_gutenberg.js', array( 'wp-blocks', 'wp-element', 'wp-i18n', 'wp-components' ), $arpricelite_assset_version );

		wp_enqueue_script( 'arprice_lite_script_for_gutenberg' );

		$pricing_table = $wpdb->prefix . 'arplite_arprice';

		$pricing_table_data = $wpdb->get_results($wpdb->prepare('SELECT ID,table_name FROM `' . $pricing_table . '` WHERE is_template != %d ORDER BY ID DESC', 1)); //phpcs:ignore
	...
	function arplite_front_assets( $force = false ) {
		...
		if ( ( isset( $arp_load_js_css ) && $arp_load_js_css == 'arp_load_js_css' ) || true === $force ) {
			...
			if ( true === $force ) {
				$arp_enqueue_func        = 'wp_print_styles';
				$arp_enqueue_func_script = 'wp_print_scripts';
			}

			wp_enqueue_script( 'arplite_front_js', ARPLITE_PRICINGTABLE_URL . '/js/arprice_front.js', array(), $arpricelite_assset_version );
			$arp_enqueue_func_script( 'arplite_front_js' );
		...
		if ( ! is_admin() ) {
			...
			if ( is_array( $posts ) ) {
				...
				foreach ( $posts as $wp_post ) {
					if ( preg_match_all( '/' . $pattern . '/s', $wp_post->post_content, $matches ) && array_key_exists( 2, $matches ) && in_array( 'ARPLite', $matches[2] ) ) {
						$frm_ids[] = $matches;
					}
				}
	...
	function arplite_front_inline_css_callback( $template_id, $tbl_preview = 0, $print_styles = false, $arplite_elementor = false, $is_gutenberg = false) {
		...
		if ( '' != $template_id ) {
			...
			if ( 1 == $tbl_preview ) {
				$post_values   = !empty( $_REQUEST['optid'] ) ? get_option( sanitize_text_field( $_REQUEST['optid'] ) ) : '';
				...
				$id                = !empty( $_REQUEST['tbl'] ) ? intval( $_REQUEST['tbl'] ) : 0;
				$table_id 		   = $id;
			...
			if ( get_option( 'arplite_desktop_responsive_size' ) && get_option( 'arplite_desktop_responsive_size' ) > 0 && $general_option['column_settings']['is_responsive'] == 1 ) {
			...
			if ( get_option( 'arplite_mobile_responsive_size' ) && get_option( 'arplite_mobile_responsive_size' ) > 0 && $general_option['column_settings']['is_responsive'] == 1 ) {
				$arplite_front_inline_css .= '
			...
			if ( get_option( 'arplite_mobile_responsive_size' ) && get_option( 'arplite_mobile_responsive_size' ) > 0 ) {
	...
	function arplite_add_tour_guide_js() {
		...
		$data2 .= '__FINISH_TEXT = "' . sprintf( esc_html__( 'Once you click save button all your changes will be saved as clone of existing template. you can right away copy short code and put it on page. %s Thank you', 'arprice-responsive-pricing-table' ), '</br>') . '";'; //phpcs:ignore
        $data2 .=';';
	...
	function arplite_hide_update_notice_to_all_admin_users() {
		if ( isset( $_GET ) && ( isset( $_GET['page'] ) && preg_match( '/arp*/', sanitize_text_field( $_GET['page'] ) ) ) ) {
	...
	function arplite_recursive_sanitize_data( $posted_data ) {
		global $arplite_pricingtable;
		...
		if ( preg_match( '/^(\d+)$/', $posted_data ) ) {
			...
			return wp_kses( $posted_data, $arplite_allowed_html );
	...
	function arplite_json_decode( $arr ) {
		$temp_val = json_decode( $arr, true );
