...
if ( ! function_exists( 'arp_get_pricing_table_string_editor' ) ) {
	...
	function arp_get_pricing_table_string_editor( $table_id, $pricetable_name = '', $is_tbl_preview = 0, $general_option = '', $opts = '', $is_clone = '' ) {
		...
		} elseif ( $is_tbl_preview && $is_tbl_preview == 3 ) {
		...
		$tablestring .= "<div class='pricingtable_menu_belt' style='display:block;'>";
			...
			$tablestring .= "<div class='pricingtable_menu_inner'>";
				...
				$tablestring .= "<div class='pricing_table_btns'>";
					...
					$display = ( empty( $id ) || $is_clone == 1 ) ? 'display:none;' : '';

					$shortcode_display = ( !empty( $_GET['arp_action'] ) && $_GET['arp_action'] == 'edit' ) ? '' : 'display:none;';
		...
		if ( $template_settings['features']['is_animated'] == 0 && $ref_template != 'arplitetemplate_23' && $ref_template != 'arplitetemplate_21' ) {
			...
			} elseif ( $column_settings['column_box_shadow_effect'] == 'shadow_style_2' ) {
				$shadow_span_text = 'Style 2';
			} elseif ( $column_settings['column_box_shadow_effect'] == 'shadow_style_3' ) {
				$shadow_span_text = 'Style 3';
			} elseif ( $column_settings['column_box_shadow_effect'] == 'shadow_style_4' ) {
				$shadow_span_text = 'Style 4';
			} elseif ( $column_settings['column_box_shadow_effect'] == 'shadow_style_5' ) {
		...
		} elseif ( '' === trim( $arp_template_skin_selected_key ) ) {
		...
		foreach ( $skins_reference_template_colors as $skins_reference_template_color ) {
			...
			if ( $skins_reference_template_color == 'Multicolor' ) {
				...
				$tablestring .= ' ';
				$tablestring .= '</li>';
			} elseif ( 'db_custom_skin' == $skins[ $reference_template ]['skin'][ $arp_cntr ] ) {
		...
		if ( isset( $column_animation['is_animation'] ) && $column_animation['is_animation'] == 'yes' && $column_animation['is_pagination'] == 1 && ( $column_animation['pagination_position'] == 'Top' || $column_animation['pagination_position'] == 'Both' ) ) {
		...
		if ( $hover_type == 0 && $is_tbl_preview != 2 ) {
			$hover_class = 'hover_effect';
		} elseif ( $hover_type == 1 && $is_tbl_preview != 2 ) {
		...
		if ( isset( $column_animation['is_animation'] ) && $column_animation['is_animation'] == 'yes' && $is_tbl_preview != 2 && $is_tbl_preview != 3 ) {
		...
		if ( $opts['columns'] && count( $opts['columns'] ) > 0 ) {
			...
			foreach ( $opts['columns'] as $j => $columns ) {
				...
				if ( $columns['is_caption'] == 1 && $template_feature['caption_style'] == 'default' ) {
					...
					if ( $column_highlight && $column_highlight == 1 && $is_table_preview != 2 ) {
			...
			foreach ( $new_arr['columns'] as $j => $columns ) {
				...
				if ( $columns['is_caption'] == 0 ) {
					...
					$column_highlight = $opts['columns'][ $j ]['column_highlight'];
					if ( $column_highlight && $column_highlight == 1 && $is_tbl_preview != 2 ) {
						$highlighted_column = 'column_highlight ';
					} else {
						$highlighted_column = '';
					}
					...
					if ( isset( $columns['ribbon_setting'] ) && $columns['ribbon_setting'] && $columns['ribbon_setting']['arp_ribbon'] != '' && $columns['ribbon_setting']['arp_ribbon_content'] != '' ) {
					...
					if ( $template_feature['header_shortcode_position'] == 'default' && ( $ref_template == 'arplitetemplate_2' || $ref_template == 'arplitetemplate_5' || $ref_template == 'arplitetemplate_26' ) ) {
					...
					if ( $columns['is_caption'] == 1 ) {
						...
						if ( $template_feature['button_position'] == 'position_1' ) {
							...
							if ( isset( $columns['footer_content'] ) && $columns['footer_content'] != '' && $columns['footer_content_position'] == 1 ) {
								$footer_content_above_btn = 'display:block;';
							} else {
								$footer_content_above_btn = 'display:none;';
							}
					...
					for ( $ri = 0; $ri <= $maxrowcount; $ri++ ) {
						...
						if ( $template_feature['caption_style'] == 'style_1' && $template_feature['list_alignment'] != 'default' ) {
							...
							$li_class     = $ref_template . '_' . $j . '_row_' . $ri;
							$tablestring .= "<li data-template_id='" . $ref_template . "' data-level='body_li_level_options' data-type='other_columns_buttons' data-column='main_" . $j . "' class='arpbodyoptionrow arp_" . $j . '_row_' . $row_count . ' ' . $cls;
							$tablestring .= ' ' . $li_class . "' data-row-id='arp_row_" . $ri . "'  id='arp_row_" . $ri . "' style='text-align:";
							$tablestring .= "' >";
							$tablestring .= "<span class='' ";
							$tablestring .= " title='";
							$tablestring .= "'>";
					...
					if ( $template_feature['column_description'] == 'enable' && $template_feature['column_description_style'] == 'after_button' ) {
						$tablestring .= "<div class='column_description " . $title_cls . " column_description_first_step toggle_step_first' data-column='main_" . $j . "' data-template_id='" . $ref_template . "' data-type='other_columns_buttons' data-level='column_description_level'>" . stripslashes_deep( $columns['column_description'] ) . '</div>';
					}
		...
		$tablestring .= '</div>';
		$tablestring .= '</div>';
		if ( isset( $column_animation['is_animation'] ) && $column_animation['is_animation'] == 'yes' && $is_tbl_preview != 2 && $column_animation['is_pagination'] == 1 && ( $column_animation['pagination_position'] == 'Bottom' || $column_animation['pagination_position'] == 'Both' ) ) {
